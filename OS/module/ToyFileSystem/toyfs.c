...
static int toyfs_do_create(struct inode *dir, struct dentry *dentry, umode_t mode)
{
	...
	printk("start toyfs_do_create, mode : 0x%x\n", mode);
	sb = dir->i_sb;
	...
	if (S_ISDIR(mode))
	{
		printk("create dir inode\n");
	}
	else if (S_ISREG(mode))
	{
		printk("create file inode\n");
	}
...
{
	(void) excl;
	return toyfs_do_create(dir, dentry, S_IFREG | mode);
}
...
int toyfs_rmdir(struct inode *dir, struct dentry *dentry)
{
	...
	printk("start toyfs_rmdir\n");
    blk->busy = 0;
	printk("finish toyfs_rmdir\n");
